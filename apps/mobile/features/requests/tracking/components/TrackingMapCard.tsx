...
type Props = {
  emergencyCoordinate: Coordinate | null;
  responderCoordinate: Coordinate | null;
...
export function TrackingMapCard({
  ...
  return (
    <View style={[styles.card, isPreview && styles.previewCard]}>
      <MapboxGL.MapView
        style={styles.map}
        styleURL={
          satellite
      ...
      >
        ...
        {routeFeature ? (
          <MapboxGL.ShapeSource id="trackingRouteSource" shape={routeFeature}>
            <MapboxGL.LineLayer
              id="trackingRouteLine"
              style={{
                lineColor: isPreview ? "#1688E8" : "#DC2626",
                lineWidth: isPreview ? 4.5 : 4,
                lineOpacity: 0.86,
                lineCap: "round",
                lineJoin: "round",
              }}
        ...
        {emergencyCoordinate ? (
          <MapboxGL.MarkerView coordinate={emergencyCoordinate} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={[styles.emergencyPulse, isPreview && styles.previewEmergencyPulse]}>
              <View style={[styles.emergencyPin, isPreview && styles.previewEmergencyPin]}>
                <Ionicons name="warning" size={isPreview ? 14 : 17} color="#FFFFFF" />
              </View>
            </View>
          </MapboxGL.MarkerView>
        ) : null}
        ...
        {responderCoordinate ? (
          <MapboxGL.MarkerView coordinate={responderCoordinate} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={[styles.responderPin, isPreview && styles.previewResponderPin]}>
              <Ionicons name="car-sport" size={isPreview ? 13 : 15} color="#FFFFFF" />
            </View>
          </MapboxGL.MarkerView>
        ) : null}

        {!isPreview && userCoordinate ? (
          <MapboxGL.MarkerView coordinate={userCoordinate}>
      ...
      {!isPreview ? (
        <>
          <View style={styles.liveBadge}>
            <View style={[styles.liveDot, closed && styles.closedDot]} />
            <View>
              <Text style={styles.liveLabel}>{closed ? "UPDATED" : "LIVE"}</Text>
              <Text style={styles.liveTime}>Updated {lastUpdatedText}</Text>
            </View>
          </View>
          <Pressable
          ...
          <Pressable
            onPress={() => void showMyLocation()}
            style={styles.locationButton}
            accessibilityRole="button"
          >
            <Ionicons name="locate-outline" size={19} color="#334155" />
          ...
          <View style={styles.zoomControls}>
            ...
            >
              <Ionicons name="add" size={21} color="#111827" />
            </Pressable>
            ...
            >
              <Ionicons name="remove" size={21} color="#111827" />
            </Pressable>
          </View>
        </>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  previewCard: { height: "100%", marginHorizontal: 0, marginTop: 0, borderRadius: 13, shadowOpacity: 0, elevation: 0 },
  ...
  previewEmergencyPulse: { width: 46, height: 46, borderRadius: 23, borderWidth: 6 },
  previewEmergencyPin: { width: 30, height: 30, borderRadius: 15 },
  previewResponderPin: { width: 30, height: 30, borderRadius: 15 },
});
