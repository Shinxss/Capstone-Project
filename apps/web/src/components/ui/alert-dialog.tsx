...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn("fixed inset-0 z-50 bg-white/40 backdrop-blur-[1px] dark:bg-black/25", className)}
    {...props}
    ref={ref}
  />
));
...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 rounded-2xl border border-white/70 bg-background/95 p-6 shadow-2xl ring-1 ring-black/5 backdrop-blur supports-backdrop-filter:bg-background/90 dark:border-white/10 dark:ring-white/10",
        className
      )}
      {...props}
    />
