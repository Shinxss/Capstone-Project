...
function SelectShell({
  ...
  return (
    <label className="relative block min-w-0">
      <span className="absolute -top-2 left-3 z-10 bg-white px-1 text-[10px] font-medium text-slate-500">{label}</span>
...
const selectClass =
  "h-11 w-full appearance-none rounded-lg border border-slate-200 bg-white pl-9 pr-8 text-xs font-medium text-slate-800 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100";
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(280px,2fr)_1fr_1fr_1.3fr_1fr_auto] xl:items-center">
        <label className="relative block">
          ...
          <input
            ...
            className="h-11 w-full rounded-lg border border-slate-200 bg-white pl-10 pr-3 text-xs text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
          />
        </label>
        ...
        <div className="relative grid grid-cols-2 rounded-lg border border-slate-200 bg-white">
          <CalendarDays size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-600" />
          <span className="absolute -top-2 left-3 z-10 bg-white px-1 text-[10px] font-medium text-slate-500">Date Range</span>
          <input
            type="date"
            aria-label="Start date"
            value={filters.dateFrom}
            onChange={(event) => onChange((previous) => ({ ...previous, dateFrom: event.target.value }))}
            className="h-11 min-w-0 border-r border-slate-200 bg-white pl-9 pr-1 text-[10px] text-slate-700 outline-none focus:bg-blue-50"
          />
          <input
            type="date"
            aria-label="End date"
            ...
            className="h-11 min-w-0 bg-white px-1 text-[10px] text-slate-700 outline-none focus:bg-blue-50"
          />
        </div>
        ...
        <button
          type="button"
          onClick={onClear}
          className="inline-flex h-11 items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-4 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
        >
          <RotateCcw size={15} />
          Clear Filters
        </button>
