...
const statusStyles: Record<Volunteer["status"], { dot: string; pill: string; label: string }> = {
  available: {
    dot: "bg-emerald-500",
    pill: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:border-emerald-500/25",
    label: "Available",
  },
  busy: {
    dot: "bg-orange-500",
...
export default function DispatchRespondersModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-lg rounded-xl bg-white shadow-lg border border-gray-200 dark:bg-[#0B1220] dark:border-[#162544]">
        <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between dark:border-[#162544]">
          <div>
            <div className="text-sm font-extrabold text-gray-900 dark:text-slate-100">Dispatch Responders</div>
        ...
        <div className="p-5">
          <div className="text-xs font-bold text-gray-700 mb-2 dark:text-slate-300">Available responders</div>

          <div className="max-h-85 overflow-y-auto rounded-xl border border-gray-200 dark:border-[#162544]">
            {volunteers.length === 0 ? (
              <div className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">No responders found.</div>
            ) : (
          ...
          <div className="mt-4 flex gap-2">
            <button
              type="button"
              onClick={onClose}
              className="w-1/2 rounded-lg border border-gray-300 py-2 font-semibold text-gray-700 hover:bg-gray-50 dark:border-[#22365D] dark:text-slate-300 dark:hover:bg-[#122036]"
            >
              Cancel
            </button>
            <button
