...
export default function ResponderAvailabilityBadge({ responder }: { responder: DispatchableResponder }) {
  if (responder.isAwaitingResponse) {
    return <span className="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300"><Circle size={7} fill="currentColor" />Awaiting Response</span>;
  }
