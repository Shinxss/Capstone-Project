...
export default function LguTasksCompleted() {
  const vm = useLguTaskHistory("VERIFIED");
  ...
  return (
    <LguShell title="Tasks" subtitle="Verified emergency dispatches (completed)">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <div className="md:col-span-4 rounded-lg border border-gray-200 bg-white p-3 dark:bg-[#0B1220] dark:border-[#162544]">
          <div className="text-xs font-semibold text-gray-500 uppercase dark:text-slate-400">Volunteer name</div>
          <input
            value={vm.filters.volunteer}
            onChange={(e) => vm.setFilters((p) => ({ ...p, volunteer: e.target.value }))}
      ...
      {vm.error ? (
        <div className="mt-4">
          <InlineAlert variant="error" title="Tasks (Completed)">
            {vm.error}
          </InlineAlert>
        </div>
      ) : null}
                ...
                <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-[#0E1626]">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="mt-0.5 text-xs text-gray-600 dark:text-slate-400">
                      Emergency ID: <span className="font-mono">{t.emergency?.id || "-"}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-semibold text-gray-900 dark:text-slate-100">{t.volunteer?.name || "Volunteer"}</div>
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-700 dark:text-slate-300">
                    ...
                    <div className="mt-0.5 text-[11px] text-gray-500 dark:text-slate-500">
                      ({Number(t.emergency?.lat || 0).toFixed(5)}, {Number(t.emergency?.lng || 0).toFixed(5)})
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-gray-700 dark:text-slate-300">
        ...
        {selected ? (
          <div className="space-y-4">
            ...
            {selected.emergency?.notes ? (
              <div className="rounded-xl border border-gray-200 p-4 dark:border-[#162544]">
                <div className="text-sm font-extrabold text-gray-900 dark:text-slate-100 mb-2">Emergency Notes</div>
            ...
            {selected.chainRecord ? (
              <div className="rounded-xl border border-gray-200 p-4 dark:border-[#162544]">
                <div className="text-sm font-extrabold text-gray-900 dark:text-slate-100 mb-2">Audit (Blockchain)</div>
